...
class ItemAssignmentStore:
    ...
    async def list_for(
        self, tracked_item_id: int, role: ActorRole | None = None
    ) -> Sequence[ItemAssignment]:
        query = select(ItemAssignment).where(ItemAssignment.tracked_item_id == tracked_item_id)
        if role is not None:
            query = query.where(ItemAssignment.role_type == role)
        return (await self._session.scalars(query)).all()

    async def replace(
        self, *, tracked_item_id: int, role: ActorRole, actors: Iterable[Actor]
    ) -> tuple[str, ...]:
        """Make the stored assignments match GitHub, returning the logins that are new.

        Stale people are removed so a reassigned pull request stops listing whoever used to be
        on it.
        """
        wanted = {actor.login.lower() for actor in actors}
        existing = {row.github_username: row for row in await self.list_for(tracked_item_id, role)}
        ...
        added = tuple(sorted(wanted - set(existing)))
        for login in added:
            self._session.add(
                ItemAssignment(
