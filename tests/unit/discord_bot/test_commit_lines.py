...
class TestWhatOneCommitLineSays:
    def test_a_commit_with_a_body_reads_as_three_lines(self) -> None:
    ...
    def test_a_commit_with_no_body_leaves_the_line_out_rather_than_rendering_it_blank(self) -> None:
        """Most commits are this shape. An empty quote between the subject and the numbers would
        be on nearly every line this bot ever posts."""
        said = formatting.format_commit(
    ...
    def test_a_commit_with_no_message_at_all_falls_back_to_its_short_sha(self) -> None:
        ...
        said = formatting.format_commit(commit(""))
...
class TestTheNumbersUnderIt:
    def test_one_file_is_one_file(self) -> None:
        said = formatting.format_commit(commit("Tidy", changed_files=1))
    ...
    def test_anything_else_is_files(self, count: int) -> None:
        said = formatting.format_commit(commit("Tidy", changed_files=count))
...
class TestNobodyIsPinged:
    ...
    def test_markdown_in_a_subject_cannot_restyle_the_lines_under_it(self) -> None:
        ...
        said = formatting.format_commit(commit("Fix **/*.py again"))
...
class TestWhenSomebodyWritesTooMuch:
    def test_a_subject_one_character_over_the_limit_is_cut(self) -> None:
        """The limit is load-bearing rather than tidiness. `fit` drops WHOLE LINES from the end,
        so a subject longer than the message limit takes the numbers underneath it down with it,
        and the thread gets half a title and nothing else."""
        said = formatting.format_commit(commit("W" * (COMMIT_TITLE_LIMIT + 1)))

        assert said.splitlines()[0].endswith("W" * COMMIT_TITLE_LIMIT + "…")

    def test_a_subject_exactly_at_the_limit_is_left_alone(self) -> None:
        said = formatting.format_commit(commit("W" * COMMIT_TITLE_LIMIT))
    ...
    def test_the_worst_commit_anybody_could_write_still_fits_in_a_discord_message(self) -> None:
        """Against the literal 2000 rather than the constant, which is the point: asserting
        against `MESSAGE_LIMIT` compares the output to the very number that decided it.
        ...
        assert len(formatting.format_commit(worst)) < 2000
        assert len(formatting.format_commit(worst)) <= MESSAGE_LIMIT


class TestABranchThatWasRewritten:
    def test_it_names_who_did_it_and_says_why_nothing_else_follows(self) -> None:
        said = formatting.format_force_push(OCTOCAT)
    ...
    def test_a_pusher_github_does_not_know_is_still_reported(self) -> None:
        said = formatting.format_force_push(None)
